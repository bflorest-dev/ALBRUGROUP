...
export interface SubtipParentChange extends SubtipAction {
  targetTipUid: string;
}
...
export class TipEditorComponent {
  ...
  @Input({ required: true }) etapaOptions: { value: EtapaCatalogo; label: string }[] = [];
  @Input({ required: true }) estadoPostventaOptions: string[] = [];
  @Input({ required: true }) parentOptions: { label: string; value: string }[] = [];
  ...
  @Output() subtipParentChange = new EventEmitter<SubtipParentChange>();
