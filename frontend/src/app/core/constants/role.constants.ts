export const ROLE_HOME_ROUTES: Record<string, string> = {
  ADMINISTRADOR: '/app/admin/personal',
  RRHH: '/app/rrhh/postulantes',
  RECLUTADOR: '/app/reclutador',
  CAPACITADOR: '/app/capacitador',
  ASESOR_GTR: '/app/gtr',
  SUPERVISOR_GTR: '/app/gtr',
  ASESOR_VENTAS: '/app/asesor-ventas',
  SUPERVISOR_VENTAS: '/app/supervisor-ventas',
  ASESOR_BACKOFFICE: '/app/backoffice',
  SUPERVISOR_BACKOFFICE: '/app/backoffice',
  ASESOR_POSTVENTA: '/app/postventa',
  SUPERVISOR_POSTVENTA: '/app/postventa',
  COMMUNITY: '/app/community',
  MONITOR: '/app/monitor'
};
