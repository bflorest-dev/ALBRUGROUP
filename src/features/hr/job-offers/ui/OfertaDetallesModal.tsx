...
export function OfertaDetallesModal({ oferta, isOpen, onClose }: OfertaDetallesModalProps): ReactElement {
  const totalVacantes =
    oferta.cantidadInicial + oferta.ampliaciones.reduce((sum, amp) => sum + amp.cantidad, 0);

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={oferta.puestoObjetivo} size="lg" className="rrhh-modal-theme">
