...
export function OfertaCard({ oferta, index = 0, onEstadoActualizado, onOpenDetails, onOpenAmpliar }: OfertaCardProps): ReactElement {
  ...
  const delayClass = `delay-${Math.min(index, 10) * 60}`;

  return (
    <div
      className={`animate-fadeSlideUp ${delayClass} flex min-h-[220px] flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-md`}
    >
      {error && (
