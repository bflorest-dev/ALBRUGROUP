...
export function useRegistrarEmpleadoConContrato() {
  ...
  const registrar = async (payload: RegistrarEmpleadoConContratoPayload) => {
    ...
    try {
      ...
      // ═════════════════════════════════════════════════════════════════════
      // PASO 2: Registrar Contrato (usa el idEmpleado devuelto en la respuesta)
      // ═════════════════════════════════════════════════════════════════════
      try {
        const normalizedPostulacionId = Number(payload.contratoData.idPostulacion);
        const contratoPayload: RegistrarContratoRequest = {
          ...payload.contratoData,
          idPostulacion: Number.isFinite(normalizedPostulacionId) && normalizedPostulacionId > 0
            ? normalizedPostulacionId
            : undefined,
          fechaFin: payload.contratoData.fechaFin?.trim() ? payload.contratoData.fechaFin : undefined,
        };

        const contratoResponse = await ContractService.registerContract(empleadoId, contratoPayload);
