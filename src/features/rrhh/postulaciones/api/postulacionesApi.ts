...
function normalizePostulacionResponse(raw: any): PostulacionResponse {
  return {
    ...
    tipificacion: raw.tipificacion
      ...
      : null,
    etapaProceso: raw.etapaProceso ?? raw.etapa ?? '',
    estadoProceso: raw.estadoProceso ?? raw.estado ?? '',
    estadoBandeja: raw.estadoBandeja ?? raw.estado_bandeja ?? '',
    idOfertaLaboral:
...
function normalizePostulacionArray(rawArray: any[]): PostulacionResponse[] {
  return rawArray.map(normalizePostulacionResponse);
}
...
export async function obtenerPostulaciones(): Promise<PostulacionResponse[]> {
  const response = await recruitmentHttp.get<PostulacionResponse[]>('/postulaciones');
  return normalizePostulacionArray(response.data);
}
