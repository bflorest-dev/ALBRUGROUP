...
const getBadgeVariant = (label: string): 'success' | 'warning' | 'error' | 'info' | 'default' | 'secondary' | 'outline' | 'destructive' | null | undefined => {
  const value = label.toLowerCase();
  if (value.includes('aprobado') || value.includes('completado')) return 'success';
  if (value.includes('rechazado')) return 'error';
          ...
          {postulaciones.map((post) => (
            <tr key={post.id} className="hover:bg-slate-50">
              ...
              <td className="px-4 py-4 text-sm">
                <Badge variant={getBadgeVariant(post.estadoProceso || 'info')}>
                  {post.estadoProceso || 'Desconocido'}
                </Badge>
              </td>
              <td className="px-4 py-4 text-sm">
                <Badge variant="default">
                  {post.estadoBandeja || 'Sin estado'}
                </Badge>
              </td>
