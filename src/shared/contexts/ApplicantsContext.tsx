...
import { EVENT_NAMES, dispatchAppEvent } from '@shared/types';
import { loadApplicantsFromStorage, saveApplicantsToStorage, loadEmployeesFromStorage, saveEmployeesToStorage } from '@shared/lib';
 ...
 */
(globalThis as any).__applicantsGlobalSyncVersion = (globalThis as any).__applicantsGlobalSyncVersion ?? 0;
(globalThis as any).__employeesGlobalSyncVersion = (globalThis as any).__employeesGlobalSyncVersion ?? 0;
...
export const ApplicantsProvider: React.FC<ApplicantsProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Incrementar versiÃ³n global para que listeners que se monten despuÃ©s sepan que hubo un cambio
    (globalThis as any).__applicantsGlobalSyncVersion = ((globalThis as any).__applicantsGlobalSyncVersion ?? 0) + 1;
  ...
  const updateApplicant = useCallback((id: string, applicant: Applicant) => {
    setApplicants(prev => 
      prev.map(app => (app.id.toString() === id.toString() ? applicant : app))
    );
  }, []);

  const deleteApplicant = useCallback((id: string) => {
    setApplicants(prev => 
      prev.filter(app => app.id.toString() !== id.toString())
    );
  }, []);
  ...
  const updateEmployee = useCallback((id: string, employee: Applicant) => {
    setEmployees(prev =>
      prev.map(emp => (emp.id.toString() === id.toString() ? employee as unknown as Employee : emp))
    );
  }, []);
  ...
  const hireApplicant = useCallback((applicant: Applicant, employee: Employee) => {
    removeApplicant(applicant.id.toString());
