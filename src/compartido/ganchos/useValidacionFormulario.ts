 ...
 * @example
 * ```tsx
 * const { errors, validate, clearError } = useValidacionFormulario(
 *   applicantSchema,
 *   formData,
 ...
 * ```
 */
export const useValidacionFormulario = <T extends Record<string, any>>(
 ...
 * Integra useValidacionFormulario con gestión de datos
 *
 * @example
 * ```tsx
 * const { formData, handleChange, errors, validate } = useFormularioValidado(
 *   { nombres: '', apellidos: '' },
 *   applicantSchema
 * );
 *
 ...
 * ```
 */
export const useFormularioValidado = <T extends Record<string, any>>(
  initialData: T,
  schema: z.ZodSchema<T>,
  options?: {
    mode?: 'onChange' | 'onBlur' | 'onSubmit';
  ...
  const validation = useValidacionFormulario(schema, formData, { mode });
...
export const useValidatedForm = useFormularioValidado;
