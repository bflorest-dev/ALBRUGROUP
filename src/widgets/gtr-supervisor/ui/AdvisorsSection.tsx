...
export const AdvisorCard: React.FC<AdvisorCardProps> = ({
  ...
  const statusClass = (status: string) => {
    ...
    return `advisor-status-${normalized || 'default'}`;
  };

  const progressClass = (status: string) => {
    const normalized = status.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
    return `gtr-progress-fill gtr-progress-${normalized || 'default'}`;
  };
  ...
  return (
    <div className="advisor-card">
      <div className="advisor-header">
        ...
        <div className={`advisor-status-badge ${statusClass(advisor.status)}`}>
          {advisor.status}
        </div>
      </div>
      ...
      <div className="progress-container">
        <div className="gtr-progress-bar">
          <progress
            className={`gtr-progress-fill ${progressClass(advisor.status)}`}
            max={100}
